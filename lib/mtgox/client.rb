...
module MtGox
  class Client
    ...
    def buys
      post('/code/getOrders.php', pass_params)['orders'].select do |o|
        o['type'] == ORDER_TYPES[:buy]
      end.each do |o|
        o['status'] = STATUS_TYPES.invert[o['status'].to_i]
      end
    end
    ...
    def orders
      hash = post('/code/getOrders.php', pass_params)['orders'].each do |o|
        o['status'] = STATUS_TYPES.invert[o['status'].to_i]
        o['type'] = ORDER_TYPES.invert[o['type']]
      end.group_by(&:type)
